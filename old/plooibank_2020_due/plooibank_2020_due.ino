...
volatile const byte cha_ans = 50;
volatile const byte chb_ans = 51;
...
volatile int totalpuls_ans = 0;
volatile int prevpuls_ans = 0;
...
int mspeed, maxcomps;
int normspeed; //normale mspeed voor wanneer omhoog aan het bewegen

//          motor compensation variables            //
int master = 1;     //master-motor, 1 of 2
int mspeedi = 70;   //snelheid in % van master
int minspeedi = 15; //minimumsnelheid in % van master bij compensatie
...
void setup() { // dit stuk code wordt eens uitgevoerd wanneer de arduino is opgestart
  ...
  pinMode(cha_ans, INPUT);
  pinMode(chb_ans, INPUT);
  ...
  attachInterrupt(digitalPinToInterrupt(chr1), beginpuls, RISING);
  attachInterrupt(digitalPinToInterrupt(chr2), beginpuls2, RISING);
  attachInterrupt(digitalPinToInterrupt(cha_ans), inpuls_aanslag, RISING);
...
void loop() {
  ...
  if(geenpuls1 && geenpuls2 && millis() >= prevmili + 20){ // dit stuk code wordt uitgevoerd als er geen puls 
    mcompensate();
    ...
    if(totalpuls_ans != prevpuls_ans){
      sendint('c', totalpuls_ans);
      prevpuls_ans = totalpuls_ans;
    }
  ...
  if(comp && eainit && ebinit){
    if(dir == "up"){
      ...
      if((deltap > mindelta && deltap > 0)){
        ...
        analogWrite(mbup, normspeed - deltas);
    ...
    if(dir == "down"){
      digitalWrite(mbup, LOW);
      int deltap = totalpuls2 - totalpuls1;
      if((deltap > mindelta && deltap > 3)){
        int deltas = map(deltap, mindelta, mindelta + comprange, 0, maxcomps);
        if(deltas > maxcomps){ deltas = maxcomps; }
        analogWrite(mbdown, mspeed + deltas);
        Serial.print("compensating up with ");
        Serial.println(map(-deltas, 0, 255, 0, 100));
      }else if(deltap < -mindelta && deltap < 3){
        int deltas = map(-deltap, mindelta, mindelta + comprange, 0, maxcomps);
        if(deltas > maxcomps){ deltas = maxcomps; }
        analogWrite(mbdown, mspeed - deltas);
        Serial.print("compensating up with ");
      ...
      if(deltap > 200 && stage == 6){
        alarm("te grote delta");
      }
    }
  }
